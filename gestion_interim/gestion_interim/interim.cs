...
{
    public partial class interim : UserControl
    {
        ...
        private void interim_Load(object sender, EventArgs e)
        {
            ...
            cn.Close();
            //footer
            cmd = new MySqlCommand("SELECT * from agent,interim LIMIT 0.1", cn);
            dr = cmd.ExecuteReader();
            while (dr.Read())
            {
                ...
                string sfonction = dr.GetString("fonction");
                nom.Text = snom;
                prenom.Text = sprenom;
                code.Text = scode;
